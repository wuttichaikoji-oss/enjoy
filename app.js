...
function migrateShopName(name) {
    ...
    if (!value || LEGACY_SHOP_NAMES.includes(value))
...
function normalizeState(parsed = {}) {
    var _a, _b;
    return {
        settings: Object.assign(Object.assign(Object.assign({}, defaultState.settings), (parsed.settings || {})), { shopName: migrateShopName((_a = parsed.settings) === null || _a === void 0 ? void 0 : _a.shopName), firebaseShopCode: ((_b = parsed.settings) === null || _b === void 0 ? void 0 : _b.firebaseShopCode) || defaultState.settings.firebaseShopCode }),
...
function migrateV1(old = {}) {
    var _a;
    return Object.assign(Object.assign({}, cloneDeep(defaultState)), { settings: Object.assign(Object.assign(Object.assign({}, defaultState.settings), (old.settings || {})), { shopName: NEW_SHOP_NAME, firebaseShopCode: ((_a = old.settings) === null || _a === void 0 ? void 0 : _a.firebaseShopCode) || defaultState.settings.firebaseShopCode }), customers: old.customers || [], factories: [], prices: (old.prices || []).map((p) => ({
            id: p.id || uid('price'),
            date: p.date,
        ...
        })), purchases: old.bills || [], sales: [], stockAdjustments: [] });
}
function loadState() {
    try {
        ...
        if (v2Raw)
            return normalizeState(JSON.parse(v2Raw));
        const v1Raw = localStorage.getItem('rubberBillV1');
        if (v1Raw)
            return migrateV1(JSON.parse(v1Raw));
...
function applyFirebaseDefaults() {
    let changed = false;
    if (!state.settings.firebaseConfigText || state.settings.firebaseConfigText.indexOf('enjoy-5eff3') === -1) {
    ...
    // เวอร์ชันนี้ต้องการให้ Cloud Sync เปิดเป็นค่าเริ่มต้นทุกเครื่อง
    // ถ้าเครื่องเคยจำค่า Local only จากเวอร์ชันก่อน จะเปิดกลับให้อัตโนมัติ
    if (state.settings.cloudSyncEnabled !== true) {
...
function saveState() {
    saveLocalState();
    renderCloudStatus();
    if (!isApplyingCloud && state.settings.cloudSyncEnabled)
        scheduleCloudPush();
}
function parseFirebaseConfig(text) {
    let clean = String(text || '').trim();
    if (!clean)
        throw new Error('กรุณาวาง Firebase Config ก่อน');
    clean = clean
    ...
    try {
        ...
        if (!cfg.apiKey || !cfg.projectId)
            throw new Error('Firebase Config ต้องมี apiKey และ projectId');
        return cfg;
    }
    catch (error) {
...
function renderCloudStatus(extraText = '') {
    ...
    if (last)
        last.textContent = extraText || (state.settings.lastCloudSyncAt ? `ซิงก์ล่าสุด: ${new Date(state.settings.lastCloudSyncAt).toLocaleString('th-TH')}` : 'ยังไม่ได้ซิงก์ Cloud');
}
...
function scheduleCloudPush() {
    if (!firebaseCtx || !state.settings.cloudSyncEnabled)
        return;
...
async function pushFullStateToCloud() {
    ...
    const safeSettings = Object.assign(Object.assign({}, state.settings), { firebaseConfigText: '', updatedAt: new Date().toISOString() });
...
function subscribeCloudRealtime() {
    ...
    cloudUnsubscribes.push(fsMod.onSnapshot(metaPath(), (snap) => {
        ...
        state.settings = Object.assign(Object.assign(Object.assign(Object.assign({}, defaultState.settings), state.settings), cloudSettings), keep);
        saveLocalState();
    ...
    CLOUD_COLLECTIONS.forEach((name) => {
        ...
        cloudUnsubscribes.push(fsMod.onSnapshot(ref, (snap) => {
            isApplyingCloud = true;
            state[name] = snap.docs.map((d) => (Object.assign({ id: d.id }, d.data()))).sort((a, b) => String(b.createdAt || b.date || '').localeCompare(String(a.createdAt || a.date || '')));
            saveLocalState();
            isApplyingCloud = false;
            renderAll();
            renderCloudStatus();
...
function unsubscribeCloud() {
    cloudUnsubscribes.forEach((fn) => { try {
        fn();
    }
    catch (_) { } });
    cloudUnsubscribes = [];
...
async function pullCloudOnce() {
    ...
    if (meta.exists()) {
        ...
        state.settings = Object.assign(Object.assign(Object.assign(Object.assign({}, defaultState.settings), state.settings), meta.data()), keep);
    }
    for (const name of CLOUD_COLLECTIONS) {
...
async function cloudDeleteRecord(name, id) {
    if (!firebaseCtx || !state.settings.cloudSyncEnabled || !id)
        return;
    try {
        await firebaseCtx.fsMod.deleteDoc(cloudPath(name, id));
    }
    catch (error) {
        console.warn(error);
    }
...
function paymentLabel(method, kind = 'purchase') {
    if (kind === 'sale')
        return { cash: 'เงินสด', transfer: 'โอน', credit: 'ค้างรับ' }[method] || method;
    return { cash: 'เงินสด', transfer: 'โอน', credit: 'ค้างจ่าย' }[method] || method;
}
function statusLabel(status, kind = 'purchase') {
    if (kind === 'sale')
        return { received: 'รับเงินแล้ว', pending: 'ค้างรับ', cancelled: 'ยกเลิก' }[status] || status;
...
function applyDefaultPurchasePrice(force = false) {
    ...
    if (type === 'latex' && method === 'drc' && (force || !$('purchasePriceDrc100').value))
        $('purchasePriceDrc100').value = price.latexBuyDrc || '';
    if (type === 'cup' && method === 'kg' && (force || !$('purchasePriceKg').value))
        $('purchasePriceKg').value = price.cupBuyKg || '';
    if (type === 'cup' && method === 'drc' && (force || !$('purchasePriceDrc100').value))
        $('purchasePriceDrc100').value = price.cupBuyDrc || '';
    if (type === 'scrap' && method === 'kg' && (force || !$('purchasePriceKg').value))
        $('purchasePriceKg').value = price.cupBuyKg || '';
}
function applyDefaultSalePrice(force = false) {
    ...
    if (type === 'latex' && method === 'drc' && (force || !$('salePriceDrc100').value))
        $('salePriceDrc100').value = price.latexSaleDrc || price.latexBuyDrc || '';
    if (type === 'cup' && method === 'kg' && (force || !$('salePriceKg').value))
        $('salePriceKg').value = price.cupSaleKg || price.cupBuyKg || '';
    if (type === 'cup' && method === 'drc' && (force || !$('salePriceDrc100').value))
        $('salePriceDrc100').value = price.cupSaleDrc || price.cupBuyDrc || '';
    if (type === 'scrap' && method === 'kg' && (force || !$('salePriceKg').value))
        $('salePriceKg').value = price.cupSaleKg || price.cupBuyKg || '';
}
...
function calculatePurchaseFromForm() {
    ...
    return Object.assign(Object.assign({ grossWeight, tareWeight, method, drcPercent, priceDrc100, pricePerKg, transportDeduct, debtDeduct, otherDeduct, totalDeduct }, calc), { netPay: calc.netAmount, grossAmount: calc.grossAmount });
}
...
function fillSaleCostSuggestion(force = false) {
    ...
    if (force || !$('saleCostKg').value)
        $('saleCostKg').value = cost ? cost.toFixed(2) : '';
...
function calculateSaleFromForm() {
    ...
    return Object.assign(Object.assign({ grossWeight, tareWeight, method, drcPercent, priceDrc100, pricePerKg, saleExpense, costPerKg, costAmount, profit, netSale }, calc), { netAmount: netSale });
}
...
function generateNumber(prefix, date, collection) {
    ...
    return `${prefix}-${date.replace(/-/g, '')}-${String(count).padStart(3, '0')}`;
}
...
function validateTransaction(calc, kind) {
    if (calc.grossWeight <= 0)
        throw new Error('กรุณาใส่น้ำหนักรวม');
    if (calc.tareWeight > calc.grossWeight)
        throw new Error('หักถังมากกว่าน้ำหนักรวมไม่ได้');
    if (calc.method === 'drc') {
        if (calc.drcPercent <= 0 || calc.drcPercent > 100)
            throw new Error('กรุณาใส่ DRC ระหว่าง 0-100%');
        if (calc.priceDrc100 <= 0)
            throw new Error('กรุณาใส่ราคา DRC 100');
    }
    if (calc.method === 'kg' && calc.pricePerKg <= 0)
        throw new Error('กรุณาใส่ราคาต่อกิโลกรัม');
    if (kind === 'sale' && calc.netWeight > stockByProduct()[$('saleProduct').value].netWeight + 0.0001) {
        ...
        if (!ok)
            throw new Error('ยกเลิกการบันทึกบิลขาย');
    }
}
async function readPurchaseForm() {
    ...
    if (!customerName)
        throw new Error('กรุณาใส่ชื่อชาวสวน');
    const imageData = await imageFileToDataUrl($('purchaseImage'));
    return Object.assign(Object.assign({ id: uid('purchase'), billNo: generateNumber('PB', date, state.purchases), date, time: $('purchaseTime').value || currentTime(), customerName, productType: $('purchaseProduct').value, method: $('purchaseMethod').value }, calc), { deductNote: $('purchaseDeductNote').value.trim(), paymentMethod: $('purchasePaymentMethod').value, status: $('purchaseStatus').value, note: $('purchaseNote').value.trim(), imageData, staffName: state.settings.defaultStaff || '', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
}
async function readSaleForm() {
    ...
    if (!factoryName)
        throw new Error('กรุณาใส่ชื่อโรงงาน');
    const imageData = await imageFileToDataUrl($('saleImage'));
    return Object.assign(Object.assign({ id: uid('sale'), billNo: generateNumber('SB', date, state.sales), date, time: $('saleTime').value || currentTime(), factoryName, productType: $('saleProduct').value, method: $('saleMethod').value }, calc), { paymentMethod: $('salePaymentMethod').value, status: $('saleStatus').value, note: $('saleNote').value.trim(), imageData, staffName: state.settings.defaultStaff || '', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
}
function resetPurchaseForm(keepCustomer = false) {
    ...
    if (keepCustomer)
        $('purchaseCustomer').value = customer;
...
function ensureCustomer(name) {
    ...
    if (!exists)
        state.customers.unshift({ id: uid('customer'), name: clean, phone: '', zone: '', createdAt: new Date().toISOString() });
}
function ensureFactory(name) {
    ...
    if (!exists)
        state.factories.unshift({ id: uid('factory'), name: clean, phone: '', address: '', createdAt: new Date().toISOString() });
}
function saveCustomerFromPurchase() {
    const name = $('purchaseCustomer').value.trim();
    if (!name)
        return toast('กรุณาใส่ชื่อชาวสวนก่อน');
    ensureCustomer(name);
    saveState();
    renderCustomers();
...
function saveFactoryFromSale() {
    ...
    if (!name)
        return toast('กรุณาใส่ชื่อโรงงานก่อน');
    ensureFactory(name);
    saveState();
    renderFactories();
...
function addCustomer(event) {
    ...
    if (exists)
        return toast('มีชื่อนี้อยู่แล้ว');
...
function addFactory(event) {
    ...
    if (exists)
        return toast('มีชื่อโรงงานนี้อยู่แล้ว');
...
function deleteCustomer(id) {
    if (!confirm('ลบรายชื่อชาวสวนนี้หรือไม่?'))
        return;
...
function deleteFactory(id) {
    if (!confirm('ลบรายชื่อโรงงานนี้หรือไม่?'))
        return;
...
function saveSettings(event) {
    event.preventDefault();
    state.settings = Object.assign(Object.assign({}, state.settings), { shopName: migrateShopName($('shopName').value.trim() || NEW_SHOP_NAME), shopAddress: $('shopAddress').value.trim(), shopPhone: $('shopPhone').value.trim(), defaultStaff: $('defaultStaff').value.trim(), receiptSize: $('receiptSize').value, roundingMode: $('roundingMode').value });
    saveState();
...
function cancelPurchase(id) {
    ...
    if (!bill)
        return;
    if (!confirm(`ยกเลิกบิลซื้อ ${bill.billNo} หรือไม่?`))
        return;
...
function cancelSale(id) {
    ...
    if (!bill)
        return;
    if (!confirm(`ยกเลิกบิลขาย ${bill.billNo} หรือไม่?`))
        return;
    bill.status = 'cancelled';
    bill.updatedAt = new Date().toISOString();
    saveState();
...
function summarizePurchases(items) {
    ...
    return valid.reduce((acc, bill) => {
        ...
        if (bill.status === 'pending' || bill.paymentMethod === 'credit')
            acc.payable += bill.netPay || 0;
        if (bill.paymentMethod === 'cash')
            acc.cash += bill.netPay || 0;
        if (bill.paymentMethod === 'transfer')
            acc.transfer += bill.netPay || 0;
        return acc;
...
function summarizeSales(items) {
    ...
    return valid.reduce((acc, bill) => {
        ...
        if (bill.status === 'pending' || bill.paymentMethod === 'credit')
            acc.receivable += bill.netSale || bill.netAmount || 0;
        if (bill.paymentMethod === 'cash')
            acc.cash += bill.netSale || bill.netAmount || 0;
        if (bill.paymentMethod === 'transfer')
            acc.transfer += bill.netSale || bill.netAmount || 0;
        return acc;
...
function addStockAdjustment(event) {
    ...
    if (kg <= 0)
        return toast('กรุณาใส่จำนวนกิโลกรัม');
...
function deleteStockAdjustment(id) {
    if (!confirm('ลบรายการปรับสต๊อกนี้หรือไม่?'))
        return;
...
function renderPrices() {
    ...
    $('defaultLatexBuyDrc').value = (selected === null || selected === void 0 ? void 0 : selected.latexBuyDrc) || '';
    $('defaultCupBuyKg').value = (selected === null || selected === void 0 ? void 0 : selected.cupBuyKg) || '';
    $('defaultCupBuyDrc').value = (selected === null || selected === void 0 ? void 0 : selected.cupBuyDrc) || '';
    $('defaultLatexSaleDrc').value = (selected === null || selected === void 0 ? void 0 : selected.latexSaleDrc) || '';
    $('defaultCupSaleKg').value = (selected === null || selected === void 0 ? void 0 : selected.cupSaleKg) || '';
    $('defaultCupSaleDrc').value = (selected === null || selected === void 0 ? void 0 : selected.cupSaleDrc) || '';
...
function groupRows(items, keyGetter, summarizeFn, nameLabel) {
    ...
    return [...map.entries()].sort((a, b) => a[0].localeCompare(b[0], 'th')).map(([name, rows]) => (Object.assign(Object.assign({ name }, summarizeFn(rows)), { nameLabel })));
}
...
function renderSettings() {
    ...
    if ($('firebaseShopCode'))
        $('firebaseShopCode').value = state.settings.firebaseShopCode || defaultState.settings.firebaseShopCode;
...
function printSale(id) {
    const bill = state.sales.find((b) => b.id === id);
    if (!bill)
        return toast('ไม่พบบิลขาย');
    $('printArea').innerHTML = receiptHtml(bill, 'sale');
    window.print();
}
function rowsToCsv(rows) {
    return rows.map((row) => row.map((cell) => `"${String(cell !== null && cell !== void 0 ? cell : '').replace(/"/g, '""')}"`).join(',')).join('\n');
}
...
async function resetData() {
    ...
    state = Object.assign(Object.assign({}, cloneDeep(defaultState)), { settings: Object.assign(Object.assign({}, cloneDeep(defaultState.settings)), keepCloud) });
    saveState();
    location.reload();
}
function renderAll() {
...
function bindEvents() {
    ...
    $('pushCloudBtn').addEventListener('click', async () => { try {
        if (!firebaseCtx)
            await initFirebaseCloud();
        state.settings.cloudSyncEnabled = true;
        await pushFullStateToCloud();
        subscribeCloudRealtime();
        toast('ส่งข้อมูลขึ้น Cloud แล้ว');
    }
    catch (error) {
        toast(error.message || 'ส่งข้อมูลขึ้น Cloud ไม่สำเร็จ');
    } });
    $('pullCloudBtn').addEventListener('click', async () => { try {
        state.settings.firebaseConfigText = $('firebaseConfigText').value.trim();
        state.settings.firebaseShopCode = cleanShopCode($('firebaseShopCode').value);
        state.settings.cloudSyncEnabled = true;
        saveLocalState();
        ...
        toast(error.message || 'ดึงข้อมูลจาก Cloud ไม่สำเร็จ');
    } });
    $('quickPrintLastPurchaseBtn').addEventListener('click', () => { var _a; return printPurchase(lastPurchaseId || ((_a = state.purchases[0]) === null || _a === void 0 ? void 0 : _a.id)); });
    $('quickPrintLastSaleBtn').addEventListener('click', () => { var _a; return printSale(lastSaleId || ((_a = state.sales[0]) === null || _a === void 0 ? void 0 : _a.id)); });
    $('modalCloseBtn').addEventListener('click', closeModal);
    $('modal').addEventListener('click', (e) => { if (e.target.id === 'modal')
        closeModal(); });
    ...
    $('purchasePaymentMethod').addEventListener('change', () => { if ($('purchasePaymentMethod').value === 'credit')
    ...
    $('salePaymentMethod').addEventListener('change', () => { if ($('salePaymentMethod').value === 'credit')
...
window.addEventListener('error', function (event) {
    var message = (event && event.message) ? event.message : 'แอพโหลดไม่สำเร็จ';
    var header = $('cloudStatusHeader');
    if (header) {
        header.textContent = 'ต้อง Refresh / ล้าง Cache';
        header.classList.add('local');
    }
    var toastEl = $('toast');
    if (toastEl) {
